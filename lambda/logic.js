...
const getBirthdayData = (day, month, year, timezone) => {
  ...
  const wasBorn = moment(`${month}/${day}/${year}`, "MM/DD/YYYY")
    .tz(timezone)
    .startOf("day");
  const nextBirthday = moment(`${month}/${day}/${today.year()}`, "MM/DD/YYYY")
    .tz(timezone)
    .startOf("day");
...
const createBirthdayReminder = (
  ...
  let triggerMoment = createMoment
    .startOf("day")
    .add(daysUntilBirthday, "days");
  if (daysUntilBirthday === 0) {
    triggerMoment = createMoment.startOf("day").add(1, "years"); // reminder created on the day of birthday will trigger next year
  }
  console.log(
    "Reminder schedule",
    triggerMoment.format("YYYY-MM-DDTHH:mm:00.000")
  );

  return util.createReminder(
    createMoment,
    triggerMoment,
    timezone,
    locale,
...
const convertBirthdateToYearsOld = (person, timezone) => {
  const today = moment().tz(timezone).startOf("day");
  const wasBorn = moment(person.date_of_birth.value)
    .tz(timezone)
    .startOf("day");
...
const convertBirthdaysResponse = (
  handlerInput,
  response,
  withAge,
  timezone
  ...
  if (
    ...
    !Object.keys(response.results.bindings) > 0
  ) {
    return speechResponse;
  }
  ...
  results.forEach((person, index) => {
    ...
    if (withAge && timezone && person.date_of_birth.value) {
      speechResponse += t("TURNING_YO_MSG", {
        count: convertBirthdateToYearsOld(person, timezone),
      });
    }
